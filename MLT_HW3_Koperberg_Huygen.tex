...
% $x_i \in S|_x$ such that $x=x_i$ and $f(x_i)=1$. So, we have that $p_S(x)=0$ if and only if $h_S(x)=1$.
  ...
  $m_{\mathcal{H}}(\epsilon_1,\delta) \leq \lceil \frac{log(|\mathcal{H}| / \delta)}{\epsilon_1} $

    Let $\delta \in (0,1)$ and $0<\epsilon_1 \leq \epsilon_2<1$. Since $\mathcal{H}$ is PAC learnable, we 
\end{solution}
