...
\begin{solution}
  All positive samples lie within $R^*$. For any point $(x_1,x_2)\in \R^2:f(x_1,x_2)=1 \implies (a_1^*\leq x_1\leq \leq b_1^* \wedge a_2^*\leq x_2 \leq b_2^*)$. If there exist points $(x_1,x_2)\in S: f(x_1,x_2=1)$, then the borders of the rectangle $A$ returns, lie on such points within $R^*$. If there exist no $(x_1,x_2) \in S:f(x_1,x_2)=0 $, then $A$ will return $\emptyset \subset R^*$. In other cases let $R(S)_{(a_1',a_2',b_1',b_2')}$ be the rectangle $A$ returns, then it holds that $a_1^*\leq a_1'\leq b_1' \leq b_1^*$ and $a_2^* \leq a_2' \leq b_2' \leq b_2^*$. Now it holds that for any point $(x_1, x_2)\in R(S)$ that $a_1^*\leq a_1'\leq x_1 \leq b_1' \leq b_1^*$ and $a_2^* \leq a_2'\leq x_2 \leq b_2' \leq b_2^*$, so $(x_1,x_2) \in R(S) \implies (x_1,x_2) \in R^*)$, so $R(S) \subseteq R^*$.\\
\end{solution}
...
% Let $R(S)=R(a_1', a_2',b_1',b_2')$, then for all samples $x=(x_1,x_2)\in S$, if $f(x)=1 \implies (a_1^*\leq x_1\leq \leq b_1^* \wedge a_2^*\leq x_2 \leq b_2^*)$. 
% \TODO
\subsection*{$3$}
\TODO
